...
vi.mock('@google/qnt-cli-core', async (importOriginal) => {
  const actual =
    await importOriginal<typeof import('@google/qnt-cli-core')>();
  return {
    ...actual,
    performInit: vi.fn(),
  };
  ...
  describe('execute', () => {
        ...
        expect.objectContaining({
          ...
          status: expect.objectContaining({
            ...
            message: expect.objectContaining({
              parts: [{ kind: 'text', text: 'QNT.md already exists.' }],
            }),
          }),
        }),
      );
    ...
    describe('when handling submit_prompt', () => {
      beforeEach(() => {
        vi.mocked(performInit).mockReturnValue({
          type: 'submit_prompt',
          content: 'Create a new QNT.md file.',
        } as CommandActionReturn);
      });
      ...
      it('writes the file and executes the agent', async () => {
        ...
        expect(fs.writeFileSync).toHaveBeenCalledWith(
          path.join(mockWorkspacePath, 'QNT.md'),
          '',
          'utf8',
        );
