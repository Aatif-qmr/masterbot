...
def run_shadow_hyperopt(strategy: str) -> dict | None:
    ...
    tf_map = {
        "MeanReversionV1": "1h", "TrendFollowV1": "4h",
        "ScalpV1": "5m", "SwingV1": "15m",
        "DailyTrendV1": "1d", "MicroScalpV1": "1m"
    }
    ...
    cmd = [
        ...
        "--config", str(BASE_DIR / f"config/{strat_config}"),
    ...
    result = subprocess.run(cmd, cwd=BASE_DIR, capture_output=True, text=True, timeout=1800, env=env)
    ...
    for line in result.stdout.split("\n"):
        if "Best Sharpe" in line or "Sharpe Ratio" in line:
            try: best_sharpe = float(line.split(":")[-1].strip())
            except: pass
    ...
    show_cmd = [
        ...
        "--hyperopt-filename", latest_fthypt.name,
        "-n", "-1", "--print-json"
    ]
    ...
    bt_cmd = [
        ...
        "--datadir", data_dir,
        "--export", "trades",
        "--export-directory", str(BASE_DIR / "user_data/backtest_results")
    ]
    ...
    try:
        with open(bt_results_file) as f:
            ...
            print(f"[{strategy}] OOS Results -> Profit: {oos_profit_pct*100:.2f}%, Drawdown: {oos_drawdown*100:.2f}%")
            ...
            if oos_drawdown > MAX_DRAWDOWN_LIMIT:
                print(f"[{strategy}] ❌ REJECTED: OOS Drawdown {oos_drawdown*100:.2f}% > {MAX_DRAWDOWN_LIMIT*100:.0f}%")
                return {"success": False}
