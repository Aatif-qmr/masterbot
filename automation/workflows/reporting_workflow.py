...
def generate_quant_report(data: dict) -> str:
    ...
    if sentiment_score > 0.3:
        directives.append("Execute full size entries on TrendFollowV1 and DailyTrendV1. Ranging strategies should run with a 0.5x Kelly multiplier.")
    elif sentiment_score < -0.3:
        directives.append("Trigger safe-mode circuit breakers on MeanReversionV1 and SwingV1. Ensure BearScalpV1 is fully online to capture downside velocity.")
    else:
    ...
    if open_trades_count >= 10:
        ...
        directives.append("Monitor cluster health and ensure Tailscale link connectivity to M1/M2 remains stable for automated SCP model updates.")
...
def generate_aggregated_report(data: dict) -> str:
    ...
    if summary['closed_trades']:
        for t in summary['closed_trades'][:5]:
            stats_md += f"- {t['pair']} ({t['strategy']}): {t['close_profit']:.2%} at {t['close_date']}\n"
    else:
