...
def test_no_dca_above_threshold(dca):
    ...
    result = dca.adjust(
        trade, current_rate=0.985, min_stake=5.0, max_stake=500.0, current_profit=0.0
    )
...
def test_no_dca_price_drop_too_small(dca):
    ...
    result = dca.adjust(
        trade, current_rate=0.974, min_stake=5.0, max_stake=500.0, current_profit=-0.026
    )
    assert result is None
...
def test_first_dca_order_amount(dca):
    ...
    result = dca.adjust(
        trade, current_rate=0.97, min_stake=5.0, max_stake=500.0, current_profit=-0.03
    )
...
def test_second_dca_order_scaled(dca):
    ...
    r2 = dca.adjust(
        trade, current_rate=0.97 * 0.978, min_stake=5.0, max_stake=500.0, current_profit=-0.05
    )
...
def test_no_dca_after_max_safety_orders(dca):
    ...
    result = dca.adjust(
        trade, current_rate=rate * 0.97, min_stake=5.0, max_stake=500.0, current_profit=-0.15
    )
    assert result is None
...
def test_max_dca_multiplier_caps_total(dca):
    ...
    for _ in range(3):
        r = dca.adjust(
            trade, current_rate=rate, min_stake=5.0, max_stake=500.0, current_profit=-0.05
        )
        results.append(r)
        rate *= 0.97
...
def test_skip_if_below_min_stake():
    dca = DcaExecutor(
        safety_orders=3, price_step_pct=0.02, volume_scale=1.0, max_dca_multiplier=4.0
    )
    trade = _trade(stake=1.0, open_rate=1.0)  # tiny stake → slice < min_stake
    result = dca.adjust(
        trade, current_rate=0.97, min_stake=50.0, max_stake=500.0, current_profit=-0.03
    )
    assert result is None
...
def test_respects_max_stake(dca):
    trade = _trade(stake=100.0, open_rate=1.0)
    result = dca.adjust(
        trade, current_rate=0.97, min_stake=5.0, max_stake=80.0, current_profit=-0.03
    )
    assert result <= 80.0
...
def test_trade_without_id_raises():
    ...
    with pytest.raises(ValueError, match="id"):
        dca.adjust(
            trade_no_id, current_rate=0.97, min_stake=5.0, max_stake=500.0, current_profit=-0.03
        )
